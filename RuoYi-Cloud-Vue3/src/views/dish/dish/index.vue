      ...
      <el-form ref="dishRef" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="菜品分类" prop="categoryId">
          <el-select v-model="form.categoryId" placeholder="请选择菜品分类" style="width: 150px" @change="$forceUpdate()" clearable>
            <el-option v-for="(key,value) in categoryList" :key="value" :label="key" :value="value" />
          </el-select>
        </el-form-item>
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" icon="Plus" @click="handleAddDishFlavor"
              >添加</el-button
            >
          </el-col>
          <el-col :span="1.5">
        ...
        >
          ...
          <el-table-column label="口味名称" prop="name" width="150">
            <template #default="scope">
              ...
              >
                <el-option
                  v-for="dishFlavor in dishFlavorSelect"
                  :key="dishFlavor.name"
          ...
          <el-table-column label="口味列表" prop="value" width="150">
            <template #default="scope">
              ...
              <el-select
                ...
                placeholder="请选择口味列表"
                multiple
                @change="handleFlavorFocus(scope.row)"
              >
                <el-option
...
const data = reactive({
  ...
  rules: {
    ...
    status: [{ required: true, message: "售卖状态不能为空", trigger: "change" }],
    categoryId: [{ required: true, message: "菜品分类不能为空", trigger: "change" }],
  },
});
...
const getCategoryList = async () => {
  try {
    ...
    console.log("Names of type 0:", categoryList);
  } catch (error) {
...
function changeFlavorName(row) {
  ...
  checkValueList.value = dishFlavorSelect.value.find(
    (item) => item.name == row.name
  ).value;
}
//定义口味列表获取焦点时更新当前选中的口味列表数据
function handleFlavorFocus(row) {
  //根据选中的name去查找静态数据的value
  checkValueList.value = dishFlavorSelect.value.find(
    (item) => item.name == row.name
  ).value;
...
function submitForm() {
  proxy.$refs["dishRef"].validate((valid) => {
    if (valid) {
      form.value.categoryList = categoryList.value;
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除菜品管理编号为"' + _ids + '"的数据项？')
    .then(function () {
      return delDish(_ids);
    })
    .then(() => {
...
/** 菜品及套餐分类序号 */
function rowCategoryIndex({ row, rowIndex }) {
...
function handleDeleteCategory() {
  if (checkedCategory.value.length == 0) {
    proxy.$modal.msgError("请先选择要删除的菜品及套餐分类数据");
  } else {
    const categorys = categoryList.value;
    const checkedCategorys = checkedCategory.value;
    categoryList.value = categorys.filter(function (item) {
      return checkedCategorys.indexOf(item.index) == -1;
    });
  }
}
/** 菜品口味关系删除按钮操作 */
